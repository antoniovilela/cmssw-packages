#include "HFTowerEdmNtupleDumperAOD.h"
...
void HFTowerEdmNtupleDumperAOD::produce(edm::Event& evt, const edm::EventSetup& stp) {
	...
	bool getLabel = evt.getByLabel(calotowersLabel_, calotowercands);
	if(!getLabel) {LogTrace("") << ">>> Calo tower collection doesn't exist";return;}

	/*try {
            bool getLabel = evt.getByLabel(calotowersLabel_, calotowercands);
	    std::cout << ">>>>>>>>>>>>>>> Ahhhhhhhhhh: " << getLabel << std::endl;	
        } catch (...) {
            LogTrace("") << ">>> Calo tower collection doesn't exist";
            return;
        }*/

	//std::cout << ">>>>>>>>>>>>>>> Calo towers size: " << calotowercands->size() << std::endl;
