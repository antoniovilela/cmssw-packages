...
void setDirsDataMC(std::vector<std::pair<std::string,TDirectory*> >& dirs, std::vector<double>& normFactors){
   ...
   TFile* fileMC_PYTHIA = TFile::Open("root/analysisMinBiasFWLite_histos_MinBias_STARTUP3X_V8K_900GeV_new_highQualityTracks.root");
...
void setDirsDataMCComponents(std::vector<std::pair<std::string,TDirectory*> >& dirs, std::vector<double>& normFactors){
   ...
   normFactors.push_back(1.);
   normFactors.push_back((nEventsPreSel_Data/nEventsPreSel_All)*(nEvents_All/nEvents_SD));
   normFactors.push_back((nEventsPreSel_Data/nEventsPreSel_All)*(nEvents_All/nEvents_NonSD));
}
