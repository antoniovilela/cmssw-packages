...
WToMuNuSelector::WToMuNuSelector( const ParameterSet & cfg ) :
      ...
      useNormIso_(cfg.getUntrackedParameter<bool> ("UseNormalizedIso",false)),	
...
bool WToMuNuSelector::filter (Event & ev, const EventSetup &) {
	  ...
	  float ptsumOverpt = ptsum/mu->pt();
          LogTrace("") << "\t... Isol, Track pt, = " << mu->pt() << " GeV, " << " ptsum = " << ptsum << " ptsum/pt = " << ptsumOverpt;
	  bool passIso = (useNormIso_)?(ptsumOverpt < isoCut_):(ptsum < isoCut_);
